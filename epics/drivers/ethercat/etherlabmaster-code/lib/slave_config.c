        ...
        int32_t value)
{
    ...
    if (!io.key) {
        ecrt_errcode = ECRT_ERRSLAVECFGFLGMEM;
        ERRPRINTF("Failed to allocate %zu bytes of flag key memory.\n",
                io.key_size);
        return -ENOMEM;
    }
    ...
    if (EC_IOCTL_IS_ERROR(ret)) {
        ecrt_errcode = ECRT_ERRSLAVECFGFLGFEAT;
        ERRPRINTF("Failed to configure feature flag: %s\n",
                strerror(EC_IOCTL_ERRNO(ret)));
        return -EC_IOCTL_ERRNO(ret);
    }
